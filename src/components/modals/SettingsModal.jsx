...
const SettingsModal = ({ onClose, field, value, type, item, onConfirmLogout }) => {
    ...
    <div className='flex gap-2 sm:gap-3'>
      {value.map((v, i) => (
        <input
          ...
          className='w-full max-w-[80px] h-12 sm:h-14 border border-gray-200 rounded-lg text-center text-lg outline-none bg-gray-50'
        />
      ))}
    </div>
  )
  ...
  return (
    <div className={`fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4 ${fontFamily.main}`}>
      ...
      {type === 'details' && (
        <div className="bg-white rounded-[20px] w-full max-w-[420px] flex flex-col items-center relative shadow-xl p-6 sm:p-8 gap-6">
          ...
          <Button onClick={onClose} text='Save' width='w-full max-w-[257px]' height='h-[50px]' bg='bg-[#05062F]' className='text-white cursor-pointer rounded-lg font-semibold text-sm' />
        </div>
      )}
      ...
      {type === 'pin' && (
        <div className="bg-white rounded-[20px] w-full max-w-[425px] flex flex-col items-center relative shadow-xl p-6 sm:p-8 gap-6">
          <button onClick={onClose} className="absolute top-4 right-4 w-7 h-7 font-bold rounded-full border cursor-pointer border-black text-gray-400 flex items-center justify-center text-sm hover:bg-gray-100">✕</button>
          <h2 className="text-xl font-bold text-[#0f1c3f] text-center">Edit Transaction Pin</h2>
      ...
      {type === 'property' && item && (
        <div className="bg-white rounded-[20px] w-full max-w-[430px] flex flex-col relative shadow-xl p-6 sm:p-8 gap-5">
          ...
          <p className={`${fontSize.lg} ${fontWeight.medium} ${fontFamily.main} ${textColor.primary} pr-8`}>
            {item.title}
          </p>
          <div className='flex flex-col gap-4'>
            {propertyRows.map((row) => (
              <div key={row.label} className='flex items-center justify-between gap-3 border-b border-gray-100 pb-3'>
                <p className={`${fontSize.sm} ${fontWeight.normal} ${fontFamily.main} ${textColor.secondary}`}>
                  {row.label}
                </p>
                <p className={`${fontSize.sm} ${fontWeight.medium} ${fontFamily.main} ${textColor.primary} text-right break-all`}>
                  {row.value}
                </p>
              </div>
            ))}
