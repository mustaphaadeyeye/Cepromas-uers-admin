...
const OwnerInvestments = () => {
  ...
  // Fetch all investment packages from the database
  const {
    data: allInvestments,
    isPending,
    isError,
  ...
  const investmentList = Array.isArray(allInvestments) ? allInvestments : [];

  // Extract owner/agent profile directly from the first investment response
  const agent = useMemo(() => {
    return investmentList[0]?.agent || null;
  }, [investmentList]);

  const agentName = agent?.fullName || "Cephas Asset Management";
  ...
  const agentOccupation = agent?.occupation || "Portfolio & Asset Management";

  // Computed Portfolio Metrics
  const stats = useMemo(() => {
    const total = investmentList.length;
    const realEstate = investmentList.filter(
      (p) => p.category === "REAL_ESTATE" || !p.category,
    ).length;
    const agriculture = investmentList.filter(
      (p) => p.category === "AGRICULTURE",
    ).length;
  ...
  const filteredInvestments = useMemo(() => {
    ...
    } else if (sortBy === "MIN_HIGH") {
      result.sort(
        (a, b) => Number(b.minAmount || 0) - Number(a.minAmount || 0),
      );
    } else if (sortBy === "ROI_HIGH") {
      result.sort((a, b) => Number(b.roi || 0) - Number(a.roi || 0));
    }
    return result;
  }, [investmentList, selectedCategory, sortBy]);

  const categoriesList = [
    { key: "ALL", label: "All Packages", count: stats.total },
    { key: "REAL_ESTATE", label: "Real Estate", count: stats.realEstate },
    { key: "AGRICULTURE", label: "Agriculture", count: stats.agriculture },
  ];
    ...
    return (
      <div className="text-center p-12">
        <p className="text-red-500 font-medium">
          Failed to load investment packages from this manager.
        </p>
        <button
          onClick={() => navigate(-1)}
    ...
    <Wrapper>
      <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto font-sans pb-12">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-6 sm:p-8 shadow-xs flex flex-col gap-6">
          ...
          <div className="bg-[#F8FAFD] border border-gray-100 rounded-xl p-4 grid grid-cols-2 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x divide-gray-200/80 gap-3 text-center">
            <div className="p-2">
            ...
            <div className="p-2">
              ...
              <p className="text-xs text-gray-500 mt-0.5">Real Estate</p>
            </div>
            <div className="p-2">
              <p className="text-xl sm:text-2xl font-bold text-[#05062F]">
                {stats.agriculture}
              </p>
              <p className="text-xs text-gray-500 mt-0.5">Agriculture</p>
            </div>
            <div className="p-2">
              <p className="text-xl sm:text-2xl font-bold text-[#05062F]">
                {stats.uniqueAreas.length || 1}
              </p>
              <p className="text-xs text-gray-500 mt-0.5">Hubs covered</p>
            </div>
          </div>
        </div>

        {/* ============ 2. HUBS COVERED CAPSULE ============ */}
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-xs flex flex-col gap-3">
            <div className="flex items-center gap-2 text-xs font-semibold text-gray-700">
              ...
              <span className="text-gray-400 font-normal">
                • {stats.uniqueAreas.length} hub
                {stats.uniqueAreas.length > 1 ? "s" : ""} active
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {stats.uniqueAreas.map((area, idx) => (
                ...
                >
                  {area}{" "}
                  <strong className="text-[#05062F] font-bold">Active</strong>
                </span>
              ))}
            </div>
          </div>
        ...
        {/* ============ 3. INVESTMENT PACKAGES CATALOG SECTION ============ */}
        <div className="flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-[#05062F]">
                Investment Packages from {agentName}
              </h2>
              <p className="text-xs text-gray-400 mt-0.5">
                Showing 1–{filteredInvestments.length} of {stats.total} package
                {stats.total === 1 ? "" : "s"}
              </p>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <option value="MIN_LOW">Min Investment: Low to High</option>
                <option value="MIN_HIGH">Min Investment: High to Low</option>
              </select>
            </div>
          </div>
          ...
          {filteredInvestments.length > 0 ? (
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-12 text-center text-gray-400">
              <p className="text-sm">
                No active investment packages found under this category.
              </p>
            </div>
          )}
        </div>

        {/* ============ 4. ABOUT MANAGER INFORMATION CARD ============ */}
        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-xs flex flex-col gap-3">
          ...
          >
            {agentName} oversees capital allocation, real estate syndications,
            and structured asset growth opportunities based in {agentAddress}.
            Offering secured, asset-backed high-yield packages with scheduled
            bullet maturity timelines across Nigeria.
          </p>
